...
- Styling comes from `src/App.css`, a byte-identical copy of [`shared/styles/todo.css`](../../shared/styles/todo.css). The copy is enforced: `cd docs && npm run check:css` fails if it drifts, and `npm run sync:css` resyncs it.
...
| `src/App.css` | Byte-identical copy of the shared stylesheet (checked by `npm run check:css`) |
